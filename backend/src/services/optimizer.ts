...
export class PolyhouseOptimizer {
  ...
  private async fillAreaWithIndependentRotation(): Promise<Polyhouse[]> {
    ...
    // Adaptive grid spacing based on land area
    // LARGER grid = FEWER placement attempts = LARGER polyhouses = LOWER COST
    ...
    if (this.landArea.area > 300000) { // > 30 hectares
      ...
      gridSpacing = 20; // Increased from 15m for cost optimization
    } else if (this.landArea.area > 2000) { // > 0.2 hectares
      gridSpacing = 15; // Increased from 8m for cost optimization
    } else {
      gridSpacing = 12; // Increased from 5m - fewer, larger polyhouses
    }
    console.log(`  Using fine grid spacing: ${gridSpacing}m for ${(this.landArea.area / 10000).toFixed(2)} hectare area`);
